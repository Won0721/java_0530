...
<body>
	...
	<a href="<c:url value='/board/insert?bo_ori_num=${board.bo_num}'/>" class="btn btn-outline-warning">답글</a>
	...
	<script type="text/javascript">
		$('[name=co_contents]').focus(function(){
			if('${user.me_id}' == ''){
				if(confirm('댓글을 작성하려면 로그인해야합니다. 로그인을 하겠습니까?'))	{
					locaton.href = '<c:url value = "/member/login"/>';	
				}
				$(this).blur();
				return;	
			}
